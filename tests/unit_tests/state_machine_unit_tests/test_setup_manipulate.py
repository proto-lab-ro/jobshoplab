...
from jobshoplab.types import Time, InstanceConfig
...
def test_begin_machine_setup_stochastic(
    ...
    instance = InstanceConfig(
        ...
        machines=(machine_with_deterministic_setup_times,),  # Use the deterministic machine that has the same ID as expected
        buffers=(),
        transports=()
    )
