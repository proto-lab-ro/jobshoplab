...
def test_binary_action_jssp_obs_done(
    ...
    # Compare array field
    assert np.allclose(result_obs["current_transition"], target_simple_jssp_obs["current_transition"])
...
def test_binary_action_jssp_obs_no_op(
    ...
    target_simple_jssp_obs["current_transition"] = np.array([0.5, 0.0, 0.33], dtype=np.float32)  # [comp_id, job, comp_type]
    obs, info = env.reset()
    ...
    target_simple_jssp_obs["current_transition"] = np.array([0.5, 1/3, 0.33], dtype=np.float32)  # job=1
    ...
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            ...
            assert obs[key] == target_simple_jssp_obs[key], f"Mismatch in {key}"
    target_simple_jssp_obs["current_transition"] = np.array([0.5, 2/3, 0.33], dtype=np.float32)  # job=2
    obs, reward, termianted, truncated, info = env.step(0)
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            assert np.allclose(obs[key], target_simple_jssp_obs[key])
    ...
    target_simple_jssp_obs["current_transition"] = np.array([4/6, 0.0, 0.33], dtype=np.float32)  # job=0
    obs, reward, termianted, truncated, info = env.step(0)
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            assert np.allclose(obs[key], target_simple_jssp_obs[key])
            ...
            assert obs[key] == target_simple_jssp_obs[key], f"Mismatch in {key}"
    target_simple_jssp_obs["current_transition"] = np.array([4/6, 1/3, 0.33], dtype=np.float32)  # job=1
    obs, reward, termianted, truncated, info = env.step(0)
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            assert np.allclose(obs[key], target_simple_jssp_obs[key])
            ...
            assert obs[key] == target_simple_jssp_obs[key], f"Mismatch in {key}"
    target_simple_jssp_obs["current_transition"] = np.array([4/6, 2/3, 0.33], dtype=np.float32)  # job=2
    obs, reward, termianted, truncated, info = env.step(0)
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            assert np.allclose(obs[key], target_simple_jssp_obs[key])
    ...
    target_simple_jssp_obs["current_transition"] = np.array([5/6, 0.0, 0.33], dtype=np.float32)  # job=0
    obs, reward, termianted, truncated, info = env.step(0)
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            assert np.allclose(obs[key], target_simple_jssp_obs[key])
            ...
            assert obs[key] == target_simple_jssp_obs[key], f"Mismatch in {key}"
    target_simple_jssp_obs["current_transition"] = np.array([5/6, 1/3, 0.33], dtype=np.float32)  # job=1
    obs, reward, termianted, truncated, info = env.step(0)
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            assert np.allclose(obs[key], target_simple_jssp_obs[key])
            ...
            assert obs[key] == target_simple_jssp_obs[key], f"Mismatch in {key}"
    target_simple_jssp_obs["current_transition"] = np.array([5/6, 2/3, 0.33], dtype=np.float32)  # job=2
    obs, reward, termianted, truncated, info = env.step(0)
    for key in target_simple_jssp_obs:
        if key == "current_transition":
            assert np.allclose(obs[key], target_simple_jssp_obs[key])
...
def test_binary_array_jssp_obs(default_init_state, default_instance, config):
    ...
    expected_transition = np.array([0.5, 0.0, 0.33], dtype=np.float32)  # component_id=3 normalized to 3/6=0.5
